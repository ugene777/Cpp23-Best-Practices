...
不幸的是，C++20 的使用场景有些局限，主要机制是格式化为字符串，这往往导致我们写出如下代码：
...
为了解决这个问题，C++23 添加了 <print> 头文件，我们可以通过模块来使用它。
...
\begin{myTip}{}
std::print 有多个重载版本，以及一个名为 std::println 的辅助函数，它会自动在消息末尾添加换行符。
\end{myTip}
...
\begin{myTip}{练习}
使用 clang-tidy 的 modernize-use-std-print\footnote{\url{https://clang.llvm.org/extra/clang-tidy/checks/modernize/use-std-print.html}} 检查器来升级你现有的 printf 系列函数。
\end{myTip}
