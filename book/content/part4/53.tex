...
如果启用了所有警告，很可能会收到一个“并非所有代码路径都返回值”的警告。我们可以调用 \verb|get_name(static_cast<Values>(15))|，这除了因函数未返回值而导致未定义行为外，并不违反 C++ 标准 [dcl.enum/5]。
...
\begin{myTip}{}
上面的例子中，不应该收到“不可达代码”的警告，因为有效值的范围总是大于已定义的值。
\end{myTip}

\begin{myNotic}{}
一些现代工具可以检测到这些 default 的使用情况。
\end{myNotic}
...
std::unreachable(在 C++23 中引入)如果执行，则会显式地引发未定义行为。它可以用于 switch 语句的“不可达”情况，以确保编译器执行最优操作。
