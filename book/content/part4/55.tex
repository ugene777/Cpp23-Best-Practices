SFINAE 有点像“只写”代码。if constexpr 没有 SFINAE 那么灵活，但请尽可能使用它。

回顾一下在“在多种情况下优先使用 auto”一节中的 divide 例子：
...
现在，我们希望在进行整数除法时添加不同的行为。在 C++17 之前，我们会使用 SFINAE(“替换失败并非错误”)，若一个函数的实例化失败，就会从重载决议中移除。
...
\begin{myTip}{}
很多情况下，为了提高可读性，请考虑使用 concepts 而不是 if constexpr。
\end{myTip}
...
\begin{myNotic}{}
if constexpr 块内的代码在语法上仍然正确，if constexpr 与 \#define 不同。
\end{myNotic}
...
if constexpr、concepts 和 requires 的结合产生了一种非常强大的、类似反射的能力。
