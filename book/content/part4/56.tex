尽可能将代码移出模板。使用其他函数，使用基类。编译器仍然可以自由地选择是否将它们内联。减少模板的使用能够改善编译时间并减小二进制文件大小，这两点都有益处。这样做还能消除人们通常所说的“模板代码膨胀”(在我看来这种现象并不存在\footnote{\url{https://articles.emptycrate.com/2008/05/06/nobody_understands_c_part_5_template_code_bloat.html}})(文章格式在某个时候被破坏了，抱歉)。
...
我们拥有的用于查找二进制文件膨胀和分析编译时间的工具越来越多。请研究一下这些工具，以及平台上可用的其他工具。

在你的二进制文件上运行它们，看看你能发现什么。使用 Clang 的 -ftime-trace 时，可以了解一下 ClangBuildAnalyzer。
\end{myTip}
