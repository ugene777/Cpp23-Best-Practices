...
Lambda 最初在 C++11 中引入，起初功能受限。随着每个 C++ 版本的演进，Lambda 变得越来越灵活和强大。

Lambda 反转了 C++ 其他部分的一些默认规则。它们默认为 const，并且在可能的情况下会自动成为 constexpr。
...
如果理解了 C++ Lambda 的每一个方面以及编译器如何实现，那么就掌握了 C++ 中所有重要的知识。
...
我已经有两年没看过这段代码了，但这里有一个 Compiler Explorer 链接供你品鉴。

\url{https://godbolt.org/z/hhde3P}

\begin{myTip}{练习：理解并批判性地评价给出的例子。}
对于上面的例子，我本可以做得有什么不同？它能否用概念加以约束？是否需要更好的命名？你会怎么做？
\end{myTip}
