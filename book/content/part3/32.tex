...
\begin{myNotic}{}
注意，std::string 本身可能会在内部进行分配，从而使用堆。如果你的目标是完全不使用堆，那么你需要采取其他措施。我们的目标是避免不必要的堆分配。
\end{myNotic}
...
一些具有 C 和 Java 背景的开发者可能很难理解这一点。对于 Java 来说，是因为创建对象需要使用 new。对于 C 语言来说，是因为 C 编译器无法像 C++ 编译器那样执行相同类型的优化，这是由语言差异造成的。因此，你当前的代码中可能就存在一些不必要的堆使用情况。
\end{myTip}

\begin{myTip}{练习：运行堆分析器}
有多种堆分析工具，甚至你的 IDE 中可能就内置了一个。检查你的堆使用情况，寻找项目中对堆的潜在滥用。很可能你大部分的堆分配都来自于对 std::string 或 std::vector 等容器的意外拷贝。
\end{myTip}
...
\begin{itemize}
\item
Code::Dive 2018: Björn Fahller “What Do You Mean By Cache Friendly?”\footnote{\url{https://youtu.be/Fzbotzi1gYs}}
