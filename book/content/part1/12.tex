...
从技术上讲，这个检查本身并不构成未定义行为(UB)，但这个检查永远不可能失败。如果 this 真的等于 nullptr，那么程序就已经处于未定义行为状态了。过去人们经常这样做，但这种行为一直都未定义，不能在对象的生命周期之外访问。如今的编译器会移除这种检查。

理论上唯一能让 this 为 nullptr 的情况，可以直接在一个空对象上调用成员函数。
...
对 nullptr 的检查可能以检查 NULL 或与 0 比较的形式隐藏。对 this 是否为 NULL 的检查很可能只存在于非常古老的代码库中。请确保已启用编译警告，然后查找这些情况。

在代码库中搜索 this == 也是一个有趣的尝试，看看会发现哪些奇怪的代码。
\end{myTip}
